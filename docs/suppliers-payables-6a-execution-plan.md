...
## سياسة الإغلاق (فرّق واضح)
- **إغلاق Supplier Account:** قدرة `supplier_accounts.close` (Admin) · رصيد صفر · لا فواتير DRAFT/POSTED/PARTIALLY_PAID · يغلق قناة الفوترة للحساب.
- **إغلاق Supplier:** يتطلب أن تكون **جميع** الحسابات المالية CLOSED مسبقًا + رصيد صفر + لا فواتير مفتوحة · نهائي بلا إعادة تفعيل.
- العرض/الطباعة يبقى متاحًا للسجلات المغلقة.
