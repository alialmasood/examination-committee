...
/** 9.B.3 — حظر رفض المراجعة */
export async function auditRejectionBlocked(
  client: TxClient,
  input: {
    userId: string;
    runId: string;
  ...
  await writeFinancialAudit(client, {
    userId: input.userId,
    action: 'payroll_run.rejection_blocked',
    entityType: 'payroll_run',
    entityId: input.runId,
    newValues: {
      operation: 'REJECT',
...
/** 9.B.3 — فشل تقني أثناء الرفض */
export async function auditRejectionFailed(
  client: TxClient,
  input: {
    userId: string;
    runId: string;
  ...
  await writeFinancialAudit(client, {
    userId: input.userId,
    action: 'payroll_run.rejection_failed',
    entityType: 'payroll_run',
    entityId: input.runId,
    newValues: {
      operation: 'REJECT',
    ...
    description:
      'حدث خطأ تقني أثناء رفض مراجعة الرواتب. بقيت حالة التشغيل دون تغيير.',
    ipAddress: input.ip,
    userAgent: input.ua,
  });
}
