...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Emergency System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم rgency ...');
    const user = await upsertUser(
      'rgency',
      'rgency123',
      'نظام اللجنة الامتحانية - قسم تقنيات طب الطوارئ',
      'rgency@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام EMERGENCY ...');
    const system = await upsertSystem('EMERGENCY', 'نظام اللجنة الامتحانية - قسم تقنيات طب الطوارئ', '/emergency');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: rgency123 — النظام: ${system.code} — المسار: http://localhost:3000/emergency`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
