...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Health System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم health ...');
    const user = await upsertUser(
      'health',
      'health123',
      'نظام اللجنة الامتحانية - قسم تقنيات صحة المجتمع',
      'health@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام HEALTH ...');
    const system = await upsertSystem('HEALTH', 'نظام اللجنة الامتحانية - قسم تقنيات صحة المجتمع', '/health');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: health123 — النظام: ${system.code} — المسار: http://localhost:3000/health`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
