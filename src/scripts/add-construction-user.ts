...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Construction System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم construction ...');
    const user = await upsertUser(
      'construction',
      'construction123',
      'نظام اللجنة الامتحانية - قسم هندسة تقنيات البناء والانشاءات',
      'construction@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام CONSTRUCTION ...');
    const system = await upsertSystem('CONSTRUCTION', 'نظام اللجنة الامتحانية - قسم هندسة تقنيات البناء والانشاءات', '/construction');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: construction123 — النظام: ${system.code} — المسار: http://localhost:3000/construction`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
