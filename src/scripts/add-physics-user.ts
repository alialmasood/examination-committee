...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Physics System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم physics ...');
    const user = await upsertUser(
      'physics',
      'physics123',
      'نظام اللجنة الامتحانية - قسم تقنيات الفيزياء الصحية',
      'physics@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام PHYSICS ...');
    const system = await upsertSystem('PHYSICS', 'نظام اللجنة الامتحانية - قسم تقنيات الفيزياء الصحية', '/physics');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: physics123 — النظام: ${system.code} — المسار: http://localhost:3000/physics`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
