...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Optics System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم optics ...');
    const user = await upsertUser(
      'optics',
      'optics123',
      'نظام اللجنة الامتحانية - قسم تقنيات البصريات',
      'optics@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام OPTICS ...');
    const system = await upsertSystem('OPTICS', 'نظام اللجنة الامتحانية - قسم تقنيات البصريات', '/optics');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: optics123 — النظام: ${system.code} — المسار: http://localhost:3000/optics`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
