...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Oil & Gas System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم oil ...');
    const user = await upsertUser(
      'oil',
      'oil123',
      'نظام اللجنة الامتحانية - قسم تقنيات هندسة النفط والغاز',
      'oil@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام OIL ...');
    const system = await upsertSystem('OIL', 'نظام اللجنة الامتحانية - قسم تقنيات هندسة النفط والغاز', '/oil');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: oil123 — النظام: ${system.code} — المسار: http://localhost:3000/oil`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
