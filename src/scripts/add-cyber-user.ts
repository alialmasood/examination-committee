...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Cyber Security System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم cyber ...');
    const user = await upsertUser(
      'cyber',
      'cyber123',
      'نظام اللجنة الامتحانية - قسم هندسة تقنيات الامن السيبراني والحوسبة السحابية',
      'cyber@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام CYBER ...');
    const system = await upsertSystem('CYBER', 'نظام اللجنة الامتحانية - قسم هندسة تقنيات الامن السيبراني والحوسبة السحابية', '/cyber');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: cyber123 — النظام: ${system.code} — المسار: http://localhost:3000/cyber`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
