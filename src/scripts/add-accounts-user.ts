...
async function upsertSystem(code: string, nameAr: string, basePath: string) {
  const res = await query(
    `INSERT INTO student_affairs.systems (code, name, name_ar, base_path, is_active)
     ...
     RETURNING id, code;`,
    [code, 'Accounts System', nameAr, basePath]
  );
  return res.rows[0];
}
...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم accounts ...');
    ...
    console.log('🧩 إنشاء/تحديث النظام ACCOUNTS ...');
    const system = await upsertSystem('ACCOUNTS', 'نظام الحسابات', '/accounts');
