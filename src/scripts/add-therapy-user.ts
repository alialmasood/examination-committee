...
async function main() {
  try {
    ...
    console.log('👤 إنشاء/تحديث المستخدم therapy ...');
    const user = await upsertUser(
      'therapy',
      'therapy123',
      'نظام اللجنة الامتحانية - قسم تقنيات العلاج الطبيعي',
      'therapy@college.edu'
    );

    console.log('🧩 إنشاء/تحديث النظام THERAPY ...');
    const system = await upsertSystem('THERAPY', 'نظام اللجنة الامتحانية - قسم تقنيات العلاج الطبيعي', '/therapy');

    console.log('🔗 ربط المستخدم بالنظام ...');
    await linkUserSystem(user.id, system.id);

    console.log('🎉 تم إنجاز المهمة بنجاح!');
    console.log(`📋 المستخدم: ${user.username} — كلمة المرور: therapy123 — النظام: ${system.code} — المسار: http://localhost:3000/therapy`);
  } catch (e) {
    console.error('❌ خطأ:', e);
    process.exitCode = 1;
  } finally {
