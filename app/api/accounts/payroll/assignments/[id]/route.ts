...
import {
  deletePayrollAssignment,
  loadPayrollAssignment,
  serializePayrollAssignment,
  updatePayrollAssignment,
} from '@/src/lib/accounts/payroll-assignments';
...
export async function DELETE(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertPayrollCapability(client, auth.user.id, PAYROLL_CAPABILITIES.MANAGE_ASSIGNMENTS);
      const deleted = await deletePayrollAssignment(client, {
        id,
      ...
      await writeFinancialAudit(client, {
        userId: auth.user.id,
        action: 'payroll_assignment.deleted',
        entityType: 'payroll_assignment',
        entityId: id,
        oldValues: serializePayrollAssignment(deleted),
        description: `حذف تكليف رواتب ${deleted.assignment_code}`,
