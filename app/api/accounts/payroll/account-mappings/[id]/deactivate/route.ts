...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await setPayrollAccountMappingActive(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, active: false });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_account_mapping.deactivated', entityType: 'payroll_account_mapping', entityId: id, newValues: serializePayrollAccountMapping(updated), description: `إيقاف ربط حسابات رواتب ${updated.mapping_code}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
