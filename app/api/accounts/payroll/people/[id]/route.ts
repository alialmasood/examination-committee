...
import {
  ...
  updatePayrollPerson,
} from '@/src/lib/accounts/payroll-people';
...
export async function DELETE(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertPayrollCapability(client, auth.user.id, PAYROLL_CAPABILITIES.MANAGE_PEOPLE);
      const deleted = await deletePayrollPerson(client, {
        id,
        userId: auth.user.id,
      ...
      await writeFinancialAudit(client, {
        userId: auth.user.id,
        action: 'payroll_person.deleted',
        entityType: 'payroll_person',
        entityId: id,
        oldValues: serializePayrollPerson(deleted),
        description: `حذف شخص رواتب ${deleted.person_code}`,
