...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await setPayrollPersonStatus(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, target: 'SUSPENDED' });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_person.suspended', entityType: 'payroll_person', entityId: id, newValues: serializePayrollPerson(updated), description: `إيقاف شخص رواتب ${updated.person_code}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
