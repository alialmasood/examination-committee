...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await setPayrollComponentAssignmentActive(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, active: true });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_component_assignment.activated', entityType: 'payroll_component_assignment', entityId: id, newValues: serializePayrollComponentAssignment(updated), description: `تفعيل إسناد مكوّن رواتب`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
