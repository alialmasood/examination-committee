...
import { copyPreviousMonthAmounts } from '@/src/lib/accounts/payroll-disbursement';
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const detail = await withTransaction(async (client) => {
      ...
      const result = await copyPreviousMonthAmounts(client, {
        sheetId: id,
        userId: auth.user.id,
      ...
      await writeFinancialAudit(client, {
        userId: auth.user.id,
        action: 'payroll_disbursement.copy_previous',
        entityType: 'payroll_disbursement_sheet',
        entityId: id,
        newValues: result.summary,
        description: `نسخ مبالغ الشهر السابق إلى كشف ${result.sheet.category_label}`,
