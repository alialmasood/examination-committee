...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await transitionPayrollContract(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, action: 'cancel' });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_contract.cancelled', entityType: 'payroll_contract', entityId: id, newValues: serializePayrollContract(updated), description: `إلغاء عقد رواتب ${updated.contract_number}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
