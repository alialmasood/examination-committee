...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await transitionPayrollContract(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, action: 'terminate' });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_contract.terminated', entityType: 'payroll_contract', entityId: id, newValues: serializePayrollContract(updated), description: `إنهاء عقد رواتب ${updated.contract_number}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
