...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const reason = requiredReason(body.reason, 'سبب إنهاء العقد');
    const row = await withTransaction(async (client) => {
      await assertPayrollCapability(client, auth.user.id, PAYROLL_CAPABILITIES.MANAGE_CONTRACTS);
      const updated = await transitionPayrollContract(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, action: 'terminate', reason });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_contract.terminated', entityType: 'payroll_contract', entityId: id, newValues: { ...serializePayrollContract(updated), transition_reason: reason }, description: `إنهاء عقد رواتب ${updated.contract_number} — السبب: ${reason}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
