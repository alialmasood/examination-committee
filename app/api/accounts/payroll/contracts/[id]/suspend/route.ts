...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      ...
      const updated = await transitionPayrollContract(client, { id, userId: auth.user.id, version: body.version, updated_at: body.updated_at, action: 'suspend' });
      await writeFinancialAudit(client, { userId: auth.user.id, action: 'payroll_contract.suspended', entityType: 'payroll_contract', entityId: id, newValues: serializePayrollContract(updated), description: `إيقاف عقد رواتب ${updated.contract_number}`, ipAddress: auth.ipAddress, userAgent: auth.userAgent });
      return updated;
    });
