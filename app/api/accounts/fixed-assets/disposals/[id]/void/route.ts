...
import { serializeAssetDisposal, voidAssetDisposal } from '@/src/lib/accounts/asset-disposals';
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertFixedAssetsCapability(client, auth.user.id, FIXED_ASSETS_CAPABILITIES.DISPOSAL_VOID);
      return voidAssetDisposal(client, {
        id,
        userId: auth.user.id,
