...
import { postDepreciationRun, serializeDepreciationRun } from '@/src/lib/accounts/asset-depreciation';
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertFixedAssetsCapability(client, auth.user.id, FIXED_ASSETS_CAPABILITIES.DEP_POST);
      return postDepreciationRun(client, {
        id,
        userId: auth.user.id,
