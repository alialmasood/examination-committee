...
import { cancelFixedAsset, serializeFixedAsset } from '@/src/lib/accounts/fixed-assets';
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertFixedAssetsCapability(client, auth.user.id, FIXED_ASSETS_CAPABILITIES.ASSET_CANCEL);
      return cancelFixedAsset(client, {
        id,
        userId: auth.user.id,
