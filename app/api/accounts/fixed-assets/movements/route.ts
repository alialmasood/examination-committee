...
import { createAssetMovement, listAssetMovements, serializeAssetMovement } from '@/src/lib/accounts/asset-movements';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await withTransaction((client) => listAssetMovements(client, {
    ...
    return jsonSuccess({
      data: result.rows.map(serializeAssetMovement),
      pagination: {
        page: result.page,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertFixedAssetsCapability(client, auth.user.id, FIXED_ASSETS_CAPABILITIES.MOVEMENT_PREPARE);
      return createAssetMovement(client, { ...body, created_by: auth.user.id });
    });
    return jsonSuccess({ data: serializeAssetMovement(row) }, 201);
  } catch (error) {
    return error instanceof AccountsHttpError ? jsonError(error.message, error.status) : mapPgError(error);
  }
}
