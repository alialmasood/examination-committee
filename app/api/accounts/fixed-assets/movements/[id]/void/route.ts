...
import { serializeAssetMovement, voidAssetMovement } from '@/src/lib/accounts/asset-movements';
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  try {
    ...
    const row = await withTransaction(async (client) => {
      await assertFixedAssetsCapability(client, auth.user.id, FIXED_ASSETS_CAPABILITIES.MOVEMENT_VOID);
      return voidAssetMovement(client, {
        id,
        userId: auth.user.id,
