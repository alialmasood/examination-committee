...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const voucherType = sp.get('voucher_type')?.trim().toUpperCase() || null;
    const bankAccountId = sp.get('bank_account_id');

    let receiptFilter = '';
    if (voucherType === 'BANK_RECEIPT') {
      receiptFilter = 'AND ba.allows_receipts = TRUE';
    } else if (voucherType === 'BANK_PAYMENT') {
      receiptFilter = 'AND ba.allows_payments = TRUE';
    }
    ...
    const postingAccounts = await query(
    ...
    if (bankAccountId) {
      bookBalance = await withTransaction((client) =>
        calculateBankAccountBookBalance(client, bankAccountId)
      );
    }
    ...
    return jsonSuccess({
      data: {
        ...
        banks: banks.rows,
        voucher_types: [
          { code: 'BANK_RECEIPT', name_ar: 'سند قبض مصرفي' },
          { code: 'BANK_PAYMENT', name_ar: 'سند صرف مصرفي' },
        ],
        statuses: [
