...
export default function StudentsPage() {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/department-students/xrays');
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event) => {
      if (event.data.type === 'payment-updated' && event.data.systemPath === 'xrays') {
        fetchStudents();
      }
    };
    ...
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600">قسم تقنيات الأشعة - الطلبة المسددون للأقساط</p>
        </div>
