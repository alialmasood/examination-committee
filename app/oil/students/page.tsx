...
export default function StudentsPage() {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/department-students/oil');
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event) => {
      if (event.data.type === 'payment-updated' && event.data.systemPath === 'oil') {
        fetchStudents();
      }
    };
    ...
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600">قسم تقنيات هندسة النفط والغاز - الطلبة المسددون للأقساط</p>
        </div>
