...
export default function AdminStaffPage() {
  ...
  const [terminateRow, setTerminateRow] = useState<AdminStaffRow | null>(null);
  const [terminateReason, setTerminateReason] = useState('');
  const [terminateError, setTerminateError] = useState('');
  const [deleteRow, setDeleteRow] = useState<AdminStaffRow | null>(null);
    ...
    const r =
      modalMode === 'edit' && editing
        ...
        : await fetchJson(API.people, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ person_type: 'EMPLOYEE', ...payload }),
          });
    setSaving(false);
    if (!r.__ok) {
  ...
  return (
    <main dir="rtl" className="p-4 w-full">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-bold text-gray-800">الكادر الوظيفي</h1>
        <button
          type="button"
          onClick={openCreate}
        ...
        >
          ...
          >
            <div className="border-b border-gray-200 bg-amber-800 px-4 py-3 text-white">
              <h2 className="text-base font-bold">إنهاء خدمة موظف</h2>
            </div>
            <div className="space-y-3 p-4">
              <p className="text-sm text-gray-700">
                سيتم إنهاء خدمة: <span className="font-semibold">{terminateRow.full_name_ar}</span>
              </p>
              <div>
                <label className={labelClass}>
        ...
        >
          ...
          >
            <div className="border-b border-gray-200 bg-red-950 px-4 py-3 text-white">
              <h2 className="text-base font-bold">حذف موظف</h2>
            </div>
            <div className="space-y-3 p-4">
              <p className="text-sm text-gray-700">
